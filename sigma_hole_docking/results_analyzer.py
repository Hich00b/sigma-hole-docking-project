...
class SigmaHoleResultsAnalyzer:
    ...
    def load_results(self, csv_path: str,
                    ...
                    id_col: str = 'compound_id') -> pd.DataFrame:
        """
    ...
    def rank_compounds(self, df: Optional[pd.DataFrame] = None,
                      energy_col: str = 'binding_energy_kcalmol',
                      id_col: str = 'compound_id') -> pd.DataFrame:
        """
    ...
    def get_top_hits(self, df: Optional[pd.DataFrame] = None,
                    n: int = 10,
                    energy_col: str = 'binding_energy_kcalmol') -> pd.DataFrame:
        """
    ...
    def calculate_summary_statistics(self, df: Optional[pd.DataFrame] = None,
                                   energy_col: str = 'binding_energy_kcalmol') -> Dict:
        """
    ...
    def validate_geometry(self, ligand_pdbqt: str, receptor_pdbqt: str,
                         halogen: str = 'I',
                         ...
                         steric_clash_cutoff: float = 2.0) -> Dict:
        """
        ...
        try:
            logger.info(f"Validating geometry for ligand {ligand_pdbqt} and receptor {receptor_pdbqt}")
            ...
            validation['halogen_position'] = [
                halogen_atom['x'], halogen_atom['y'], halogen_atom['z']
            ]

            if not dummy_atom:
                validation['error'] = f"No dummy atom (H with positive charge) found in ligand"
                return validation

            validation['dummy_found'] = True
            validation['dummy_position'] = [
                dummy_atom['x'], dummy_atom['y'], dummy_atom['z']
            ]
            ...
            validation['oxygen_position'] = [
                closest_oxygen['x'], closest_oxygen['y'], closest_oxygen['z']
            ]
            ...
            if grid_box_center is not None and grid_box_size is not None:
                ...
                all_within = True
                for atom in ligand_atoms:
                    if not (min_x <= atom['x'] <= max_x and
                           min_y <= atom['y'] <= max_y and
                           min_z <= atom['z'] <= max_z):
                        all_within = False
                        break
                    ...
                    for rec_atom in receptor_atoms:
                        ...
                        if distance < steric_clash_cutoff:
                            validation['steric_clashes'].append({
                                ...
                                'distance': distance,
                                'ligand_position': [lig_atom['x'], lig_atom['y'], lig_atom['z']],
                                'receptor_position': [rec_atom['x'], rec_atom['y'], rec_atom['z']]
                            })
            ...
            if validation['dummy_found'] and validation['oxygen_position'] is not None:
                ...
                if validation['halogen_position'] is not None:
                    ...
                    if dummy_to_ox_norm > 0 and halogen_to_ox_norm > 0:
                        ...
                        validation['dummy_orientation_valid'] = dot_product > 0.5  # Reasonable threshold
            ...
            if validation['halogen_found'] and validation['dummy_found'] and validation['oxygen_position'] is not None:
                ...
                if validation['halogen_position'] is not None:
                    ...
                    for atom in ligand_atoms:
                        if atom['element'] == 'C':
                            ...
                            dist = np.sqrt((atom['x'] - hx)**2 + (atom['y'] - hy)**2 + (atom['z'] - hz)**2)
                            # Typical C-X bond lengths: C-F ~1.3Å, C-Cl ~1.7Å, C-Br ~1.9Å, C-I ~2.1Å
                            if dist < min_c_dist and dist < 2.5:  # Reasonable upper bound for C-X bond
                                min_c_dist = dist
                                carbon_atom = atom

                if carbon_atom is not None and validation['halogen_position'] is not None and validation['oxygen_position'] is not None:
                    ...
                    if norm_xc > 0 and norm_xo > 0:
                        ...
                        validation['angle_valid'] = angle_deg >= 160.0  # Allow some deviation from linear
                    else:
            ...
            validation['valid'] = (
                validation['halogen_found'] and
                validation['dummy_found'] and
                validation['charge_valid'] and
                validation['distance_valid'] and
                validation['grid_box_valid'] and
                validation['steric_clash_valid'] and
                validation['dummy_orientation_valid'] and
            ...
            if validation['distance'] is not None:
                logger.info(f"Halogen-Oxygen distance: {validation['distance']:.2f} Å "
                          f"(expected: {low}-{high} Å)")
            if validation['grid_box_containment'] is not None:
                logger.info(f"Grid box containment: {'PASS' if validation['grid_box_valid'] else 'FAIL'}")
            ...
            if 'dummy_orientation_dot_product' in validation:
                logger.info(f"Dummy-Oxygen orientation dot product: {validation['dummy_orientation_dot_product']:.3f}")
            if validation['cx_o_angle'] is not None:
                logger.info(f"C-X···O angle: {validation['cx_o_angle']:.1f}° "
                          f"(expected: ≥160.0°)")
    ...
    def generate_ranking_report(self, df: Optional[pd.DataFrame] = None,
                              output_dir: str = "ranking_reports") -> Dict[str, str]:
        """
    ...
    def compare_with_physics_model(self, ligand_pdbqt: str, receptor_pdbqt: str,
        ...
        physics_scores = []
        docking_scores = df['binding_energy_kcalmol'].values if 'binding_energy_kcalmol' in df.columns else []
        ...
        valid_mask = ~(np.isnan(df['physics_energy_kcalmol']) | np.isnan(df['binding_energy_kcalmol']))
        if valid_mask.sum() > 2:
        ...
        comparison = {
            ...
            'mean_difference': float(np.nanmean(df['physics_energy_kcalmol'] - df['binding_energy_kcalmol'])) if valid_mask.sum() > 0 else None,
            'rmse': float(np.sqrt(np.nanmean((df['physics_energy_kcalmol'] - df['binding_energy_kcalmol'])**2))) if valid_mask.sum() > 0 else None
        }
        ...
        if comparison['correlation'] is not None:
            ...
            compounds_with_physics = comparison['compounds_with_physics']
            logger_info = f"Physics model comparison: {compounds_with_physics} compounds, no correlation"
            logger.info(logger_info)

        return comparison

    def generate_interaction_visualizations(self,
                                          receptor_pdbqt: str,
                                          ligand_dir: str,
                                          output_dir: str = "visualizations",
                                          num_visualizations: int = 5) -> List[str]:
        """
        ...
        try:
            visualizer = SigmaHoleInteractionVisualizer()
    ...
    def generate_publication_figures(self, df: Optional[pd.DataFrame] = None,
        ...
        try:
            ...
            plt.figure(figsize=(10, 6))
            n, bins, patches = plt.hist(df['binding_energy_kcalmol'], bins=50,
            ...
            plt.axvline(x=-5.0, color='darkgreen', linestyle='--', linewidth=2, label='Strong binder cutoff')
            plt.axvline(x=-2.0, color='goldenrod', linestyle='--', linewidth=2, label='Moderate binder cutoff')
            ...
            bars = plt.barh(y_pos, top_20['binding_energy_kcalmol'], color=colors, edgecolor='black')
            ...
            if 'halogen' in df.columns:
                ...
                halogen_data = [df[df['halogen'] == h]['binding_energy_kcalmol'].values for h in halogens]
                ...
                plt.title('Sigma-Hole Binding Energy by Halogen Type', fontsize=14, fontweight='bold')
                plt.grid(True, alpha=0.3)
                plt.tight_layout()
...
def example_usage():
    ...
    example_data = {
        'compound_id': [
            ...
            '2-iodopyridine', '3-iodopyridine', '4-iodopyridine',
            'phenyl_iodide', 'ortho_diiodobenzene', 'meta_diiodobenzene'
        ],
        'halogen': ['I', 'Br', 'Cl', 'F', 'I', 'I', 'I', 'I', 'I', 'I'],
        'binding_energy_kcalmol': [
            -5.2, -4.1, -2.8, -1.2,  # Strong to weak binders
            -4.8, -3.9, -3.1,        # Pyridine derivatives
            -5.5, -6.2, -4.9         # Multi-iodo compounds
        ],
