...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('sigma_hole_pipeline.log'),
...
class SigmaHolePipeline:
    ...
    def run_charge_calculation(self, input_csv: str,
                             vmax_col: str = 'vmax',
                             halogen_col: str = 'halogen',
                             id_col: str = 'compound_id',
                             delta_r_col: Optional[str] = None) -> str:
        """
    ...
    def prepare_ligands(self, charged_csv: str,
                       ...
                       structure_ext: str = '.sdf') -> List[str]:
        """
        ...
        # Generate ligands
        if structure_dir:
            logger.warning("Structure directory provided — using pre-optimized DFT structures "
            ...
            logger.warning("Generating ligands from SMILES (MMFF geometry) — may not match "
    ...
    def prepare_receptor(self, receptor_input: str,
                        input_type: str = 'pdb',
                        receptor_id: str = 'receptor') -> str:
        """
    ...
    def run_docking(self, receptor_pdbqt: str,
                   scoring_method: Optional[str] = None) -> str:
        """
        ...
        ligand_files = [f for f in os.listdir(self.config['ligand_dir'])
        ...
        if center_x is None or center_y is None or center_z is None:
            computed_center_x, computed_center_y, computed_center_z = self.docking_engine.compute_receptor_center(receptor_pdbqt)
        ...
        df_results = self.docking_engine.batch_score(
    ...
    def analyze_results(self, results_csv: str) -> Dict:
        ...
        df_results = self.results_analyzer.load_results(results_csv)
        ...
        if self.config['generate_figures']:
            ...
            figure_files = self.results_analyzer.generate_publication_figures(
                df_ranked,
                output_dir=os.path.join(self.config['output_dir'], 'figures')
            )
    ...
    def validate_results(self, receptor_pdbqt: str,
        ...
        for _, row in top_hits.iterrows():
            ...
            grid_box_center = (center_x, center_y, center_z) if all(v is not None for v in [center_x, center_y, center_z]) else None
            grid_box_size = (size_x, size_y, size_z) if all(v is not None for v in [size_x, size_y, size_z]) else None
            ...
            logger.info(f"Validation for {compound_id}: {'PASS' if validation['valid'] else 'FAIL'}")

        return validation_results

    def run_full_pipeline(self, input_csv: str,
                         receptor_input: str,
        ...
        try:
            ...
            df_merged = pd.merge(df_docking, df_charged[merge_columns],
            ...
            if self.config['validation_enabled'] and validation_results:
                validation_csv_path = os.path.join(self.config['output_dir'], "validation_results.csv")
            ...
            if self.config['generate_figures'] and validation_results:
                ...
                try:
                    ...
                    viz_output_dir = os.path.join(self.config['output_dir'], 'interaction_visualizations')
                    ...
                    if viz_files:
                        logger.info(f"Generated {len(viz_files)} interaction visualizations in {viz_output_dir}")
                    else:
...
def create_example_input() -> str:
    ...
    example_data = {
        'compound_id': [
            ...
            '4-fluoroacetophenone', '4-bromoacetophenone'
        ],
        'smiles': [
            'c1ccccc1I', 'c1ccccc1Br', 'c1ccccc1Cl', 'c1ccccc1F',
            'c1ccncc1I', 'c1cccnc1I', 'c1cnccc1I',
            'CC(=O)c1ccc(F)cc1', 'CC(=O)c1ccc(Br)cc1'
        ],
        'halogen': ['I', 'Br', 'Cl', 'F', 'I', 'I', 'I', 'F', 'Br'],
        'vmax': [26.0, 19.5, 14.2, 9.8, 24.5, 23.8, 22.1, 11.2, 18.9]  # Example Vmax values (kcal/mol)
    }
...
def main():
    ...
    parser.add_argument('--input', '-i', required=True,
                       help='Input CSV file with compound data')
    parser.add_argument('--receptor', '-r', required=True,
                       help='Receptor file (PDB) or SMILES string')
    parser.add_argument('--receptor-type', choices=['pdb', 'smiles'], default='pdb',
                       help='Type of receptor input')
    parser.add_argument('--receptor-id', default='receptor',
                       help='Identifier for the receptor')

    # Column specifications
    parser.add_argument('--vmax-col', default='vmax',
                       help='Column name for Vmax values')
    parser.add_argument('--halogen-col', default='halogen',
                       help='Column name for halogen symbols')
    parser.add_argument('--id-col', default='compound_id',
                       help='Column name for compound identifiers')
    parser.add_argument('--smiles-col', default='smiles',
                       help='Column name for SMILES strings')

    # Pipeline options
    parser.add_argument('--scoring-method', choices=['vinardo', 'ad4', 'vina', 'smina', 'physics'],
                       default='vinardo', help='Scoring method to use')
    parser.add_argument('--no-physics-fallback', action='store_true',
                       help='Disable physics-based scoring fallback')
    parser.add_argument('--no-validation', action='store_true',
                       help='Disable geometry validation')
    ...
    parser.add_argument('--generate-control', action='store_true',
    ...
    parser.add_argument('--grid-center-z', type=float, default=None,
                       help='Z coordinate of grid box center for docking (Å)')
    parser.add_argument('--grid-size-x', type=float, default=None,
                       help='Size of grid box in X dimension (Å)')
    parser.add_argument('--grid-size-y', type=float, default=None,
                       help='Size of grid box in Y dimension (Å)')
    parser.add_argument('--grid-size-z', type=float, default=None,
                       help='Size of grid box in Z dimension (Å)')

    # Other options
    parser.add_argument('--output-dir', default='./sigma_hole_output',
    ...
    if results['success']:
        ...
        if 'analysis' in results and 'top_hits' in results['analysis']:
            ...
            for _, row in top_hits.head().iterrows():
                print(f"{row['compound_id']:15} {row['halogen']:2} "
