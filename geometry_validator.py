...
class GeometryValidator:
    ...
    def __init__(self, bond_tolerance: float = BOND_LENGTH_TOLERANCE,
    ...
    def validate_molecule_geometry(self, mol: Chem.Mol, halogen: str) -> Dict:
        ...
        ref_angle = REFERENCE_CXC_ANGLES['sp3']  # default
        ...
        for halogen_idx in halogen_indices:
            ...
            # Find bonded carbon
            carbon_neighbors = [
                nbr for nbr in halogen_atom.GetNeighbors()
                if nbr.GetAtomicNum() == 6
            ]

            if not carbon_neighbors:
                result['messages'].append(f"No carbon neighbor for {halogen} at index {halogen_idx}")
                continue
            ...
            carbon_neighbors_2 = [
                nbr for nbr in carbon.GetNeighbors()
    ...
    def validate_structure_file(self, structure_path: str, halogen: str,
                                  structure_format: str = 'auto') -> Dict:
        """
    ...
    def batch_validate(self, structure_dir: str, input_csv: str,
                       halogen_col: str = 'halogen',
                       id_col: str = 'compound_id',
                       structure_ext: str = '.sdf') -> pd.DataFrame:
        """
    ...
    def generate_validation_report(self, validation_df: pd.DataFrame,
        ...
        if 'details' in validation_df.columns:
            ...
            if all_bonds:
                ...
                report_lines.append(f"  Passed: {sum(1 for e in errors if e <= self.bond_tolerance)}/{len(errors)}")
                report_lines.append("")
            ...
            if all_angles:
                ...
                report_lines.append(f"  Passed: {sum(1 for e in errors if e <= self.angle_tolerance)}/{len(errors)}")
                report_lines.append("")

        # Failed files
        failed = validation_df[~validation_df['valid']] if 'valid' in validation_df.columns else pd.DataFrame()
        if not failed.empty:
